...
if (empty($_POST['date'])) {
  ...
  $_SESSION['addExerciseErrors'][] = 'Date is required';
...
#sport is NOT empty.
  ...
  if (!preg_match('/^[a-zA-Z0-9 ]+$/', $sport)) {
      #The user name had something other than characters in it.
    $_SESSION['addExerciseErrors'][] = 'Only letters allowed in sport name';
...
#workout name is not empty.
    ...
    #Make sure the workout name is safe.
  $athlete = test_input($_POST['athlete']);

  #Make sure it contains only letters.
  if (!preg_match('/^[a-zA-Z0-9 ]+$/', $athlete)) {
      #The user name had something other than characters in it.
    $_SESSION['addExerciseErrors'][] = 'Only letters allowed in Athlete name';
...
if (count($_SESSION['addExerciseErrors']) > 0) {
  if ($_SESSION['myPermission'] == 'Trainer') {
    ...
    $workoutDate = mysqli_real_escape_string($connection, $date);
    ...
    $sportToAdd = mysqli_real_escape_string($connection, $sport);
    $athleteToAdd = mysqli_real_escape_string($connection, $athlete);
